...
import csv
import numpy as np
import Hand_Track.HandTrackingModule as htm  # Ensure HandTrackingModule is updated
...
def main():
    ...
    detector = htm.handDetector(detectionCon=0.7, maxHands=1)  # Detect only 1 hand
    ...
    with open("right_hand_angles.csv", "w", newline="") as f:
        ...
        while True:
            ...
            img, handType = detector.findHands(img)  # Get Right or Left

            # 2) Get Hand Landmarks
            lmList = detector.findPosition(img, draw=False)

            # 3) Check if a right hand is detected
            if lmList and len(lmList) >= 21 and handType == "Right":
                # 4) Compute angles for each joint in 3x5 matrix form
                angle_matrix = []
                ...
                # 5) Flatten Matrix to Single Row for CSV
